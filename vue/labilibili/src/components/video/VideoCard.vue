...
<template>
    <!--1. 视频的上下布局：首页和搜索-->
    <div 
    :class="{'video-longitudinal':isUpDown, 
        'video-horizental':!isUpDown,
        'video-border':isShadow
    }"  @click="turnToDetail()">
        <!--视频本体-->
        <div class="video-wrap video-content">
            <picture v-if="!isShow" class="video-content cover-show" @mouseover="setVideoOver()" @mouseleave="setVideoOut()">
                <img v-if="videoOverview.imgUrl" v-lazy="videoOverview.imgUrl" :key="videoOverview.imgUrl" class="video-content"/>
        ...
        <!--相关信息-->
        <div class="video-detail"  :class="{'flex-column-left-max-container': !isUpDown}"
        :style="{'margin-left': isUpDown? '':'1rem',  
            'justify-content': isUpDown? '': 'center'
        }">
            <div v-if="videoOverview.videoName" class="video-detail-item video-name title-font-color">{{videoOverview.videoName}}</div>
            <div class="content-font-color video-detail-text"
            :class="{'flex-between-container': isUpDown,
                'flex-column-left-max-container': !isUpDown}">
                <span v-if="videoOverview.authorName">{{videoOverview.authorName}}</span> 
...
@mixin middle-size { // 首页和搜索页，上下布局：video-longitudinal
    width: 20rem;
    height: 14rem;
}
@mixin big-size { // 排行处的大型布局
    width: 30rem;
    height: 20rem;   
}
.video-content{
