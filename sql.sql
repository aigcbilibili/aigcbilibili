...
select * from video;
select * from user;
update video_data set play_count=0;
select * from video_data vd ;
delete from `video`  where id>0;
select * from `video` d ;
insert into user(username,password,nickname) values("111","111","Alla");
insert into danmaku(video_id,user_id,content,create_time,place) values(54,2,"测试弹幕1",now(),72);
insert into comment(video_id,user_id,parent_id,top_id,content,create_time) values(54,2,null,null,"麻辣咸鱼",now());
select * from user;
delete from play where id>0;
insert into user(nickname,intro,cover) values("鱼鱼","拯救世界","https://labilibili.com/user-cover/default.png");
select * from `dynamic` d ;
update user set nickname="咸鱼二号" where id=2;
select * from play;
select * from user;
select * from video_data;
CREATE TRIGGER de_like_count AFTER DELETE ON likes 
FOR EACH ROW
BEGIN
    UPDATE video_data
...
show triggers;
show create table dynamic_to_user    ;
select * from user;
# 视频表
CREATE TABLE `video` (
