...
public class UploadAndEditServiceImpl implements UploadAndEditService {
    ...
    public Result<Boolean> upload(UploadVideoRequest uploadVideoRequest) {
        try {
            ...
            MultipartFile coverFile = uploadVideoRequest.getVideoCover();
            ...
            String url = "http://120.55.183.155:9000/video/" + fileName;
            video.setUrl(url);
            ...
            if (coverFile != null) {
//                hasCover=true;
                InputStream coverFileInputStream = coverFile.getInputStream();
                String coverName = coverFile.getOriginalFilename();
                String imgContentType = coverFile.getContentType();
                video.setCover(coverName);
                queryWrapper.eq(Video::getCover, UUID.randomUUID().toString().substring(0, 8) + coverName);
                CompletableFuture<Void> uploadImgFuture = CompletableFuture.runAsync(() -> minioService.uploadImgFile(coverName, coverFileInputStream, imgContentType)).handle((result, ex) -> {
                    if (ex != null) {
            ...
            CompletableFuture<Void> sendNoticeFuture = CompletableFuture.runAsync(() -> {
                ...
                client.dynamicNotice(uploadVideoRequest.toDynamic(user, video));
            }).handle((result, ex) -> {
                if (ex != null) {
